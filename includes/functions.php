	...
	if($srow['tid']==1){ //商户注册
		...
		if($info){
			...
			if($sds){
				...
				$msg = '<h2>商户注册成功通知</h2>感谢您注册'.$conf['sitename'].'！<br/>您的登录账号：'.$info['email'].'<br/>您的商户ID：'.$uid.'<br/>您的商户密钥请登录后台查看<br/>'.$conf['sitename'].'官网：<a href="https://'.$host.'/" target="_blank">'.$host.'</a><br/>【<a href="https://'.$host.'/user/" target="_blank">商户管理后台</a>】';
