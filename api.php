...
if($act=='query')
{
	...
	$today=date("Y-m-d");
	$order_today=$DB->getColumn("SELECT count(*) from pre_order where uid=:uid and status=1 and date=:date", [':uid'=>$pid, ':date'=>$today]);
	$order_lastday=$DB->getColumn("SELECT count(*) from pre_order where uid=:uid and status=1 and date=:date", [':uid'=>$pid, ':date'=>$lastday]);
...
elseif($act=='settle')
{
	...
	$rs=$DB->query("SELECT * FROM pre_settle WHERE uid=:uid order by id desc limit {$offset},{$limit}", [':uid'=>$pid]);
...
elseif($act=='order')
{
	if(isset($_GET['sign']) && isset($_GET['trade_no'])){
		...
		if(empty($_GET['pid']) || empty($_GET['sign']) || md5(SYS_KEY.$pid.$trade_no.SYS_KEY) !== $_GET['sign']) exit(json_encode(['code'=>-3, 'msg'=>'verify sign failed']));
...
elseif($act=='orders')
{
	...
	if(isset($_GET['status'])){
		...
		$rs=$DB->query("SELECT A.*,B.name typename FROM pre_order A LEFT JOIN pre_type B ON A.type=B.id WHERE A.uid=:uid AND A.status=:status ORDER BY trade_no DESC LIMIT {$offset},{$limit}", [':uid'=>$pid, ':status'=>$status]);
	}else{
		$rs=$DB->query("SELECT A.*,B.name typename FROM pre_order A LEFT JOIN pre_type B ON A.type=B.id WHERE A.uid=:uid ORDER BY trade_no DESC LIMIT {$offset},{$limit}", [':uid'=>$pid]);
	}
	while($row=$rs->fetch(PDO::FETCH_ASSOC)){
...
elseif($act=='refundapi')
{
	...
	$trade_no=daddslashes($_POST['trade_no']);
	if(!is_numeric($money) || !preg_match('/^[0-9.]+$/', $money))exit(json_encode(['code'=>-1, 'msg'=>'金额输入错误']));
	if(!isset($_POST['key']) || $_POST['key']!==md5(SYS_KEY.$pid.$trade_no.SYS_KEY))exit(json_encode(['code'=>-1, 'msg'=>'密钥错误']));
